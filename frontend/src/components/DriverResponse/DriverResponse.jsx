...
export const DriverResponse = ({ pickup_location, destination, fare }) => {
  ...
  return (
    <div className="w-full h-auto md:h-[58vh] max-w-8xl flex flex-col md:flex-row bg-white rounded-xl shadow-lg overflow-hidden">
      {/* Combined header and driver responses container */}
      <div className="flex flex-col w-full md:w-[60%]">
        {/* Header section */}
        <div className="w-full bg-purple-600 text-white p-3 md:p-4">
          {/* Destination info */}
          <div className="bg-white/10 p-2 md:p-3 rounded-lg flex items-center space-x-2">
            <FaLocationArrow className="text-purple-200" />
            <div>
              <p className="text-xs text-purple-100">Your destination</p>
              <p className="text-xs md:text-sm font-medium">{destination || "Central Business District, Kolkata"}</p>
            </div>
          </div>
        </div>
        
        {/* Driver responses area with fixed height */}
        <div className="w-full flex-grow h-48 md:h-64 overflow-y-auto bg-gray-50">
          {drivers.length > 0 ? (
      ...
      <div className="w-full md:w-[40%] bg-gradient-to-b from-purple-50 to-white border-t md:border-t-0 md:border-l border-gray-100 flex flex-col">
        {/* Fare header section */}
        <div className="p-4 md:p-6 pb-3 md:pb-4">
          <div className="flex justify-between items-center mb-3 md:mb-4">
            ...
            <div className="bg-purple-100 px-2 md:px-3 py-1 md:py-2 rounded-full shadow-sm">
          ...
          <div className="mb-4 md:mb-6">
            <div className="flex items-center bg-white rounded-xl border-2 border-purple-200 focus-within:ring-4 focus-within:ring-purple-200 focus-within:border-purple-400 transition-all duration-200 shadow-sm">
              ...
              <input
                ...
                className="bg-transparent text-gray-800 text-lg md:text-xl font-medium flex-1 p-2 md:p-3 outline-none w-full"
                style={{ 
          ...
          <div className="grid grid-cols-2 gap-2 md:gap-3 mb-4 md:mb-6">
            <button 
              ...
              className="bg-white text-gray-700 py-2 md:py-3 px-3 md:px-4 rounded-xl hover:bg-gray-100 transition-colors font-medium flex items-center justify-center gap-1 md:gap-2 text-xs md:text-sm shadow-sm border border-gray-200"
            ...
            <button 
              onClick={() => handleFareChange(100)}
              className="bg-white text-purple-700 py-2 md:py-3 px-3 md:px-4 rounded-xl hover:bg-purple-50 transition-colors font-medium flex items-center justify-center gap-1 md:gap-2 text-xs md:text-sm shadow-sm border border-purple-200"
            >
        ...
        <div className="px-4 md:px-6 mb-3 md:mb-4">
          <p className="text-xs text-gray-500 mb-1 md:mb-2 font-medium">Suggested Fares</p>
        ...
        <div className="px-4 md:px-6 mt-auto pb-4 md:pb-6">
          <button 
            onClick={handleSendFare}
            disabled={drivers.length === 0}
            className={`bg-purple-600 text-white py-3 md:py-4 px-3 md:px-4 rounded-xl transition-all duration-200 font-medium text-sm md:text-base w-full flex items-center justify-center gap-2 transform ${
            drivers.length === 0 ? 
          ...
          >
              <FaPaperPlane className="text-xs md:text-sm" /> Send Fare
          </button>
          <p className="text-xs text-center text-gray-500 mt-1 md:mt-2">Drivers will be notified of your fare</p>
        </div>
      </div>
    </div>
  );
