...
def add_trip_request(request_data: TripRequestCreate, db: Session = Depends(get_session)):
...
def update_trip_status(request: UpdateTripStatusRequest, db: Session = Depends(get_session)):
